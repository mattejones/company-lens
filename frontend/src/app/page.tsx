    ...
    <div className="animate-fade-in">
      <div className="mb-12">
        ...
        <p className="text-text-secondary text-sm max-w-lg">
          Search the Companies House register, enrich with LLM-powered domain inference, and verify results through DNS, WHOIS, and content signals.
        </p>
      </div>
      ...
      <form onSubmit={handleSearch} className="mb-4 flex gap-3">
        <div className="relative flex-1">
          ...
          {loading && <div className="absolute right-3 top-1/2 -translate-y-1/2"><div className="w-4 h-4 border border-accent/40 border-t-accent rounded-full animate-spin" /></div>}
        </div>
        <button type="submit" disabled={loading} className="px-6 py-3 bg-accent text-bg-base text-xs font-mono font-500 tracking-wider rounded hover:bg-accent/90 disabled:opacity-50 transition-colors">SEARCH</button>
      </form>
        ...
        <div className="flex items-center gap-4 mb-6 p-3 bg-bg-surface border border-bg-border rounded">
          <label className="flex items-center gap-2 cursor-pointer select-none">
            <button type="button" onClick={() => setActiveOnly((v) => !v)}
              className={`w-8 h-4 rounded-full transition-colors relative ${activeOnly ? "bg-accent" : "bg-bg-border"}`}>
              <span className={`absolute top-0.5 w-3 h-3 rounded-full bg-white transition-transform ${activeOnly ? "translate-x-4" : "translate-x-0.5"}`} />
            </button>
            <span className="font-mono text-xs text-text-secondary">Active only</span>
          </label>
          <div className="w-px h-4 bg-bg-border" />
          <div className="flex items-center gap-2 flex-1">
            <span className="font-mono text-xs text-text-muted shrink-0">SIC code</span>
            <input value={sicFilter} onChange={(e) => setSicFilter(e.target.value)}
              placeholder={availableSics.slice(0, 3).join(", ") || "e.g. 62012"}
              className="flex-1 bg-bg-elevated border border-bg-border rounded px-3 py-1.5 text-xs text-text-primary placeholder-text-muted font-mono focus:outline-none focus:border-accent/40 transition-all" />
            {sicFilter && <button onClick={() => setSicFilter("")} className="text-text-muted hover:text-text-secondary font-mono text-xs">✕</button>}
          </div>
          <div className="w-px h-4 bg-bg-border" />
      ...
      <div className="p-4 flex items-start gap-4">
        <div className="min-w-0 flex-1">
          <div className="flex items-center gap-2 mb-1 flex-wrap">
            ...
            {c.type && <span className="text-[10px] font-mono px-1.5 py-0.5 rounded border border-bg-border text-text-muted">{c.type.replace(/_/g, " ")}</span>}
          </div>
          ...
          {c.sic_codes && c.sic_codes.length > 0 && (
            <div className="flex flex-wrap gap-1.5 mt-2">
              {c.sic_codes.map((sic) => (
                <span key={sic} className="font-mono text-[10px] px-1.5 py-0.5 bg-bg-elevated border border-bg-border rounded text-text-secondary">{sic}</span>
              ))}
            </div>
          )}
        </div>
        ...
        <div className="shrink-0 flex items-center gap-2 pt-0.5">
          ...
          {!job ? (
            <button onClick={onEnrich} className="px-4 py-2 border border-accent/30 text-accent text-xs font-mono rounded hover:bg-accent/10 transition-all">Enrich →</button>
...
function StatusBadge({ status }: { status?: string }) {
  ...
  return <span className={`text-[10px] font-mono px-1.5 py-0.5 rounded border ${styles[s] ?? "border-text-muted/30 text-text-muted"}`}>{s}</span>;
}
